...
@protocol SessionUploadDelegate <NSObject>
@required
- (void)sessionDidFinishUpload:(SessionUpload *)aSessionUpload withUuid:(NSString *)aUuid;
- (void)session:(SessionUpload *)aSessionUpload withUuid:(NSString *)uuid didFailWithError:(NSString *)aMessage andCode:(ServerErrorCode)aCode;
@end

@interface SessionUpload : NSObject <NSURLConnectionDelegate> {
    @private
    //__weak id <SessionUploadDelegate> delegate;
    NSString *uuid_;
    NSMutableData *result_;
    NSDictionary *jsonData_;
}

@property (nonatomic, weak) id <SessionUploadDelegate> delegate;
