...
@protocol QuizLoadDelegate <NSObject>
@required
- (void)quizLoadFinished:(QuizLoad *)aQuizLoad withQuiz:(NSDictionary *)aQuiz;
- (void)quizLoad:(QuizLoad *)aQuizLoad didFailWithError:(NSString *)aErrorStr;
@end

@interface QuizLoad : NSObject <NSURLConnectionDelegate>  {
    @private
    NSMutableData *result_;
    NSDictionary *jsonData_;
}

@property (nonatomic, weak) id <QuizLoadDelegate> delegate;
