...
void _Logger(const char *file, const char *funcName, int lineNumber, NSString *level,
    ...
    if (includeThread) {
        ...
        if (threadName) {
            log = [log stringByAppendingFormat:@"%@/", threadName];
        } else {
            log = [log stringByAppendingFormat:@"%p/", [NSThread currentThread]];
        }
    }
    ...
    if (includeTime) {
        ...
        log = [log stringByAppendingFormat:@"%@ ", datetime];
    }
    
    NSString *fileName = [[NSString stringWithUTF8String:file] lastPathComponent];
    
    if (includeLoc) {
        log = [log stringByAppendingFormat:@"%s (%@:%d) ", funcName, fileName, lineNumber];
    }
    
    log = [log stringByAppendingFormat:@"[%@] %@", level, body];
