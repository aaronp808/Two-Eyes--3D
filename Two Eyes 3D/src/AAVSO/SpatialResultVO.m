...
@implementation SpatialResultVO
...
- (void)saveAttitude:(CMAttitude *)data {
    //NSString *attStr = [NSString stringWithFormat:@"%.5f,%.5f,%.5f", data.roll, data.pitch, data.yaw];
    //LogDebug(attStr);
...
- (void)saveUserAcceleration:(CMAcceleration)data {
    //NSString *accelStr = [NSString stringWithFormat:@"%.5f,%.5f,%.5f", data.x, data.y, data.z];
    //LogDebug(accelStr);
