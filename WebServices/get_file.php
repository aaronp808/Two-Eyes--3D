...
if ( ! $file || ! $folder ) {
	not_found( 'Date and file required' );
}

$path  =  realpath( "{$config['base_upload_directory']}/{$folder}/{$file}" );

if ( ! is_file( $path ) ) {
	not_found( 'File does not exist' );
}

if ( ! preg_match( $config[ 'allowed_file_pattern' ], $path ) ) {
	not_found( 'Bad file type' );
}
